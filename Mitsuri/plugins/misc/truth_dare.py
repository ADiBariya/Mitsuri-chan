...
# Adding Telegraph image to the dare question
@app.on_message(filters.command("dareimg"))
def get_dare_with_image(client, message):
    try:
        # Make a GET request to the Dare API
        response = requests.get(dare_api_url)
        if response.status_code == 200:
            dare_question = response.json()["question"]
            # Get a random Telegraph image
            telegraph_image_url = f"https://telegra.ph/file/06cdb9edd59ac36649baa.jpg{random.choice(os.listdir('dare_images'))}"
            message.reply_photo(telegraph_image_url, caption=dare_question)
        else:
            message.reply_text("Failed to fetch a dare question. Please try again later.")
    except Exception as e:
        message.reply_text("An error occurred while fetching a dare question. Please try again later.")
